...
async function testCompleteRoomSwitching() {
  try {
    console.log('🧪 Testing Complete Room Switching Functionality...\n');
    ...
    if (availableRooms.success) {
      ...
      console.log('📋 Available rooms:', availableRooms.data.map(r => ({
        ...
        availableBedCount: r.availableBedCount,
        totalBeds: r.beds.length
      })));
      
      // Show bed details for first room
      if (availableRooms.data.length > 0) {
        const firstRoom = availableRooms.data[0];
        console.log('\n📋 Bed details for first room:', firstRoom.roomNumber);
        firstRoom.beds.forEach(bed => {
          console.log(`   Bed ${bed.bedNumber}: ${bed.isOccupied ? 'Occupied' : 'Available'}`);
        });
      }
    } else {
      ...
      if (targetBed) {
        ...
        if (switchResult.success) {
          ...
          if (updatedResident.switchHistory && updatedResident.switchHistory.length > 0) {
            ...
            console.log('📋 Latest switch:', updatedResident.switchHistory[updatedResident.switchHistory.length - 1]);
          }
          
          // Verify room bed status
          const updatedRoom = await Room.findById(targetRoom._id);
          const targetBedInRoom = updatedRoom.beds.find(b => b.bedNumber === targetBed);
          if (targetBedInRoom) {
            console.log('✅ Room bed status updated:', {
              bedNumber: targetBedInRoom.bedNumber,
              isOccupied: targetBedInRoom.isOccupied,
              occupiedBy: targetBedInRoom.occupiedBy
            });
          }
    ...
    console.log('\n✅ Complete room switching tests completed!');
